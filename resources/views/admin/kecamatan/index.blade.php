...
@section('content')
  <div class="panel">
      <div class="panel-heading">
          <h4 class="panel-title"><strong>DATA KECAMATAN</strong></h4>
          <button class="btn btn-success btn-quirk pull-right" data-toggle="modal" data-form="tambah" data-target="#kecamatan-tambah">TAMBAH</button>
          <p>Menampilkan data kecamatan.</p>
      </div>
                    ...
                    @foreach ($kecamatan as $kec)
                      <tr>
                          ...
                          <td>
                            ...
                            <a class="btn btn-sm btn-warning" data-kode="{{ $kec->kode }}" data-nama="{{ $kec->nama }}" data-target="#kecamatan-tambah" data-toggle="modal" data-form="edit" data-id="{{ $kec->id }}"><i class="fa fa-edit"></i> Edit</a>
                                @csrf
                                @method('DELETE')
...
<!-- Modal -->
<div class="modal bounceIn animated" id="kecamatan-tambah" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header panel panel-success-full">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
        <h4 class="modal-title text-white" id="myModalLabel"><i class="fa fa-edit"></i> TAMBAH KECAMATAN</h4>
      </div>
      <div class="modal-body">
          <form action="{{ route('kecamatan.store') }}" method="post" id="form-kecamatan">
            <input type="hidden" name="_method">
            @csrf
            <div class="form-group">
              <label class="control-label center-block"><strong>KODE</strong></label>
              <input type="text" class="form-control" placeholder="Isikan kode kecamatan" name="kode">
            </div>
            <div class="form-group">
              <label class="control-label center-block"><strong>NAMA</strong></label>
              <input type="text" class="form-control" placeholder="Isikan nama kecamatan" name="nama">
            </div>
      </div>
...
$('#kecamatan-tambah').on('show.bs.modal', function(e){
    ...
    if(form == "edit")
    {
      ...
      $('#form-kecamatan').attr('action', BASE_URL+'/kecamatan/'+id);
      $('#form-kecamatan').find($('input[name="_method"]')).attr('value', 'PUT');
      modal.find('.modal-title').html('<i class="fa fa-edit"></i> EDIT KECAMATAN');
    ...
    if(form == "tambah")
    {
      $('#form-kecamatan').find($('input[name="_method"]')).attr('value', 'POST');
      modal.find('.modal-title').html('<i class="fa fa-edit"></i> TAMBAH KECAMATAN');
