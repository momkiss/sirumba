...
@section('content')
  <div class="panel">
      <div class="panel-heading">
          <h4 class="panel-title"><strong>JENIS BERKAS</strong></h4>
          <button class="btn btn-success btn-quirk pull-right" data-toggle="modal" data-form="tambah" data-target="#jenis-berkas-tambah">TAMBAH</button>
          <p>Menampilkan data jenis berkas.</p>
      </div>
                  ...
                  <tbody>
                    ...
                    @foreach ($jenisberkas as $berkas)
                      <tr>
                          ...
                          <td>{{ $berkas->nama }}</td>
                          <td>
                            <form action="{{ route('jenisberkas.destroy',$berkas->id) }}" method="POST">
                            <a class="btn btn-sm btn-warning" data-kode="{{ $berkas->kode }}" data-nama="{{ $berkas->nama }}" data-target="#jenis-berkas-tambah" data-toggle="modal" data-form="edit" data-id="{{ $berkas->id }}"><i class="fa fa-edit"></i> Edit</a>
                                @csrf
                                @method('DELETE')
...
<!-- Modal -->
<div class="modal bounceIn animated" id="jenis-berkas-tambah" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
          <form action="{{ route('jenisberkas.store') }}" method="post" id="form-jenis-berkas">
            <input type="hidden" name="_method">
            @csrf
            <div class="form-group">
              <label class="control-label center-block"><strong>KODE</strong></label>
              <input type="text" class="form-control" placeholder="Isikan kode jenis berkas" name="kode">
            </div>
            <div class="form-group">
              <label class="control-label center-block"><strong>NAMA</strong></label>
              <input type="text" class="form-control" placeholder="Isikan jenis nama berkas" name="nama">
            </div>
      </div>
...
$('#jenis-berkas-tambah').on('show.bs.modal', function(e){
    ...
    if(form == "edit")
    {
      ...
      $('#form-jenis-berkas').attr('action', BASE_URL+'/jenisberkas/'+id);
      $('#form-jenis-berkas').find($('input[name="_method"]')).attr('value', 'PUT');
      modal.find('.modal-title').html('<i class="fa fa-edit"></i> EDIT JENIS BERKAS');
    ...
    if(form == "tambah")
    {
      $('#form-jenis-berkas').find($('input[name="_method"]')).attr('value', 'POST');
      modal.find('.modal-title').html('<i class="fa fa-edit"></i> TAMBAH JENIS BERKAS');
